...
let websocket = new WebSocket("wss://pxws.sstars.ws:3001/relay/");
let dateTime = new Date();
let lastTime = dateTime.getTime();

let canvas = document.getElementById('livestream');
...
async function sketchLoop() {
  ...
  if (poses[0]) {
    ...
    //console.log((controlPointX/canvas.width).toFixed(3), (controlPointY/canvas.height).toFixed(3))
    //SEND X,Y
    sendWebSocket(controlPointX, controlPointY)
...
function sendWebSocket(x,y){
  let dateTime = new Date();
  if (dateTime.getTime() - lastTime > 1){
    lastTime = dateTime.getTime();
    x = 1 - (x/canvas.width).toFixed(3)
    y = 1 - (y/canvas.height).toFixed(3)
